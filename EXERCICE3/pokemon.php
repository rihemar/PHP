...
echo "<div class='row card p-3 m-2 Comb bg-primary text-white text-center fs-4 fw-bold rounded-4 shadow-sm'>Les Combattants</div><br>";
echo "<div class='row'>";
foreach ([0, 1] as $i) {
    echo "<div class='col card p-3 m-2 bg-light border border-dark rounded-3 shadow'>";
    ...
    echo "<div>❤️ Points : <strong>{$Battlers[$i]->getHealth()}</strong></div><hr>";
...
while (!$Battlers[0]->isFainted() && !$Battlers[1]->isFainted()) {
    $round++;
    echo "<div class='round row card p-3 m-2 bg-danger text-white text-center rounded-4 shadow-sm'>Round $round";
...
// Winner section
echo "<br><br><div class='vainq p-4 text-center bg-success text-white fw-bold fs-4 rounded-4 shadow'>";
